...
const MemberCard = ({ imageURL, name, id, memberId }) => {
  return (
    <div className={classes.member__card}>
      <Link to={`/members/${id}`} className={`d-flex align-items-center pt-1 pb-2 text-white ${classes.member__link}`}>
        ...
        <span>{name}</span>
        {memberId == id ? <i className={`bi bi-dot ${classes.member__choosen}`}></i> : ''}
      </Link>
    </div>
  )
}
